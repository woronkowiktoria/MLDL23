...
class EpicKitchensDataset(data.Dataset, ABC):
    ...
    def _get_train_indices(self, record, modality='RGB'):
        ...
        indices = []
        # Select a random starting index for each clip
        for i in range(num_clips):
          start_index = random.randint(0, record.num_frames[modality] - num_frames * self.stride)
          clip_indices = [start_index + j * self.stride for j in range(num_frames)]
    ...
    def _get_val_indices(self, record, modality):
      ...
      num_clips = self.num_clips
      indices = []
    # Center the sampling around the middle of the clip
      for i in range(num_clips):
        center_index = (record.num_frames[modality] - num_frames * self.stride) // 2
        clip_indices = [center_index + j * self.stride for j in range(num_frames)]
        indices.extend(clip_indices)
      return indices
